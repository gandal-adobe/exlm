export default function decorate(block) {
  ...
  const pictureElemSubject = block.querySelector('div:nth-child(3) > div picture');
  const imageDescrSubject = block
    .querySelector('div:nth-child(4) > div')
    .textContent.trim();

  const eyebrow = block
    .querySelector('div:nth-child(5) > div')
    .textContent.trim()
    .toUpperCase();
  const title = block
    .querySelector('div:nth-child(6) > div')
    .textContent.trim();
  const longDescr = block.querySelector('div:nth-child(7) > div').innerHTML;
